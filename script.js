...
document.addEventListener('DOMContentLoaded', () => {
  // assign the button click
  document.querySelectorAll('.drpbtn').forEach(btn => {
    btn.addEventListener('click', e => {
   
      // first close all
      document.querySelectorAll('.drpdown-content').forEach(div => div.classList.remove('show'))
      // open the one you want which is CLOSEST
      e.target.closest('.drpdown').querySelector('.drpdown-content').classList.add('show');
  ...
  // catch the click outside
  document.addEventListener('click', e => {
  if (e.target.classList.contains('drpbtn')) return;
    document.querySelectorAll('.drpdown-content').forEach(div => div.classList.remove('show'))
  })
